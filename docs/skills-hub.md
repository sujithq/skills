...
In any repository where they want the hub skills available, users run:
...
For offline sharing, private handoff, or release artifacts, build a bundle from this hub:
...
Use this path when consumers cannot reach GitHub directly or when you want to hand them the exact package artifact you tested.
...
- `apm_modules/`, which is only a local dependency cache
